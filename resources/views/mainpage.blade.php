...
<div class="container-fluid header shadow animate__animated animate__fadeInDown">
    <div class="row">
        <div class="toast show d-none tost shadow" id="tost">
        ...
        <div class="col-md-2 col-10 text-start buy mt-md-3 cart-m user">
            @if(\Illuminate\Support\Facades\Auth::check())
                <span class="badge bg-danger mx-2 fa-1x" id="badgeid">
                   <?php
                    $count=\App\Models\Order::where('user_id',\Illuminate\Support\Facades\Auth::id())->count()
                    ?>
                {{$count}}
        ...
        <div class="col-md-6">
            <nav class="navitem d-none d-md-block" id="menuid">
                <ul class="casecademenu">
                    <li><a href="{{url('firstpage')}}" {{ request()->is('*firstpage*') ? 'class=active' : '' }}>خانه</a>
                    </li>
                    <li><a>محصولات</a>
                        <ul>
                            <li><a href="{{url('bread')}}" {{ request()->is('*bread*') ? 'class=active' : '' }}>نان</a>
                            </li>
                            <li><a href="{{url('cake')}}" {{ request()->is('*cake*') ? 'class=active' : '' }}>کیک</a>
                            </li>
                            <li><a href="{{url('food')}}" {{ request()->is('*food*') ? 'class=active' : '' }}>غذا</a>
                            </li>
                        </ul>
                    </li>
                    <li><a href="{{url('gallery')}}" {{ request()->is('*gallery*') ? 'class=active' : '' }}>گالری</a>
                    </li>
                    <li><a href="{{url('firstpage')}}#about">درباره ما</a></li>
                    <li><a href="#contact">تماس با ما</a></li>
                    @can('isAdmin')
                    <li><a href="{{url('admin')}}" {{ request()->is('*admin*') ? 'class=active' : '' }}>مدیر سایت</a></li>
                    @endcan
                </ul>
            </nav>
        </div>
            ...
            <div class="modal" id="myModal">
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        ...
                        <div class="modal-body" id="sabadkharid">
                            ...
                            <button type="submit" title="پرداخت" class="btn btn-outline-success w-100">پرداخت</button>
                        </div>
    ...
    <div class="row">
        ...
        <div class="col-md-6 col-12">
            ...
            <div class="text-start ms-md-5">
                ...
                <a href="http://www.twiter.com"><img src="{{url('image/twiter.png')}}" class="mx-3 wow fadeInDown"
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        $.ajax('{{url('badge/index')}}',
            {
                ...
                success: function (data, status, xhr) {   // success callback function
                    var contentIndex='';
                    for(item of data.sabad){
                        contentIndex=contentIndex+'<tr>' +
                            '<td>' + 1 + '</td>' +
                            '<td><img src="image/' +item.loc+ '.jpg' + '" class="img-fluid" width="50px"></td>' +
                            '<td>'+item.name+'</td>' +
                            '<td>'+(item.price).toLocaleString()+'</td>' +
                            '<td><input type="number" value="'+item.count+'" name="count" class="form-control" onchange="cahengecount('+item.id+',event,{{$userId}})"></td>' +
                            '<td>'+(item.totalprice).toLocaleString()+'</td>' +
                            '<td><button class="btn btn-danger" title="حذف" onclick="deletesabad('+item.id+',{{$userId}})"><i class="fa fa-trash "></i></button></td>' +
                            '</tr>';
                    }
                    $('#item').html(contentIndex);
                    $('#footer').html(
                    '<tr>'+
                        '<th colspan="5">'+
                        'مبلغ قابل پرداخت'+
                        '</th>'+
                        '<th>' + parseInt(data.sumPrice).toLocaleString()+'ریال '+'</th>'+
                    '</tr>')
        ...
        cahengecount=function (cn,e,userId){
            console.log('edit');
            ...
            $.ajax('{{url('badge/update')}}',
                {
                    dataType: 'json',
                    data: {'item_id':cn,'count':count,'user_id':userId},
                    method:'patch',
                    success: function (data, status, xhr) {   // success callback function
                        var contentEdit='';
                        for(item of data.sabad){
                            contentEdit=contentEdit+'<tr>' +
                                '<td>' + 1 + '</td>' +
                                '<td><img src="image/' +item.loc+ '.jpg' + '" class="img-fluid" width="50px"></td>' +
                                '<td>'+item.name+'</td>' +
                                '<td>'+(item.price).toLocaleString()+'</td>' +
        ...
        deletesabad=function (sadadID,userId){
            console.log('delete');
            $.ajax('{{url('badge/index')}}',
                {
                    dataType: 'json',
                    data: {'sadadID':sadadID,'user_id':userId},
                    method:'delete',
                    success: function (data, status, xhr) {   // success callback function
                        var contentDelete='';
                        for(item of data.sabad){
                            contentDelete=contentDelete+'<tr>' +
                                '<td>' + 1 + '</td>' +
                                '<td><img src="image/' +item.loc+ '.jpg' + '" class="img-fluid" width="50px"></td>' +
                                '<td>'+item.name+'</td>' +
                                '<td>'+(item.price).toLocaleString()+'</td>' +
                        ...
                        $('#item').html(contentDelete);
                        $('#badgeid').html(data.cart);
                        $('#footer').html(
                            '<tr>'+
                            '<th colspan="5">'+
                            'مبلغ قابل پرداخت'+
        ...
        addtobascket = function (vl,userId) {
            console.log('add');
            $.ajax('{{url('badge/create')}}',
                {
                    dataType: 'json',
                    data: {'product_id':vl,'user_id':userId},
                    method:'post',
                    success: function (data, status, xhr) {   // success callback function
                        for (item of data.order){
                        }
                        if (item.product_id==data.product.id){
                            $('#tost').removeClass('d-none');
                            ...
                            $.ajax('{{url('badge/add')}}',
                                {
                                    ...
                                    success: function (data, status, xhr) {   // success callback function
                                        var contentAdd='';
                                        for(item of data.sabad){
                                            contentAdd=contentAdd+'<tr>' +
                                                '<td>' + vl + '</td>' +
                                                '<td><img src="image/' +item.loc+ '.jpg' + '" class="img-fluid" width="50px"></td>' +
                                                '<td>'+item.name+'</td>' +
                                                '<td>'+(item.price).toLocaleString()+'</td>' +
                                                '<td><input type="number" name="count" value="'+item.count+'" class="form-control"></td>' +
                                                '<td>'+(item.totalprice).toLocaleString()+'</td>' +
                                                '<td><button class="btn btn-danger" title="حذف"><i class="fa fa-trash"></i></button></td>' +
                                                '</tr>';
                                        }
                                        $('#item').html(contentAdd);
                                        $('#badgeid').html(data.cart);
                                        $('#footer').html(
                                            '<tr>'+
                                            '<th colspan="5">'+
